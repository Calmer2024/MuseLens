...
- 当前 `route_and_run` 是同步等待执行完成后返回；如果后续需要真正的实时步骤推送，建议补一条 Router 专用 WebSocket 或 SSE 通道
